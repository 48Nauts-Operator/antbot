...
import antbot.channels.matrix as matrix_module
from antbot.bus.events import OutboundMessage
from antbot.bus.queue import MessageBus
from antbot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from antbot.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("antbot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "antbot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("antbot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "antbot.channels.matrix.asyncio.create_task", _fake_create_task
    )
